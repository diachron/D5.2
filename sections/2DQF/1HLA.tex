...
\item [(C) $\langle$ \texttt{ex:UniBonn rdfs:label "UniBonn"} $\rangle$] - \todo{CL@JD: Is this third example necessary?  It's redundant with the second.}The literal ("UniBonn") in the triple causes the malformed capitalisation metric to point out a problem in this triple.
\end{description}
...
This is already done in the DIACHRON project, where a number of metrics (cf. Section~\ref{sec:DQMetrics}) required to assess the various use cases specified in Deliverable 1.1~\todo{CL: add to bibtex}\cite{diachron-d1.1} are implemented.
...
\begin{description}
\item[compute] assesses the \todo{CL@JD: As this might not be clear to everyone, it wouldn't hurt to explain in one sentence what a quad is.}quad/triple which is passed by the stream processor by the defined metric;
\item[metricValue] returns the value computed by the quality metric;
\item[toDAQTriples] returns a list of daQ triples, containing quality metadata about the assessed metric, which \todo{CL@JD: really \emph{will}, or rather \emph{can}?  The QA framework is not itself in charge of storage.}will be stored in the dataset as a new named graph (quality graph);
...
\item[getQualityProblems] returns a typed (List$\langle$Resource$\rangle$ or List$\langle$Quad$\rangle$) \todo{CL@JD: I wouldn't speak of ``ProblemList'' as it wrongly suggests that there is a Java class with this name}ProblemList which will be used to create a quality report of the metric;
\end{description}
