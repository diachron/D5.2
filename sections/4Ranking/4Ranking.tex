...
Quality assessment frameworks should not be only about positive or negative assertions of a the dataset, but also it should place the publishers doubts in the assertion spectrum.
With our proposed ranking algorithm, such effects will also be given a small share of the \emph{ranking-glory}.
...
\begin{enumerate}
\item Get the total number of metrics assessed ($m$);
...
In Definition~\ref{def:weight_adjustment} we calculate a weight value for the number of metrics chosen ($f_{m}$).
Together with the number of metrics chosen, we add 1 to represent a small share which will be divided equally with the rest of the metrics ($\overline{f_{m}}$ represents the metrics not chosen).
...
\begin{align*}
\rho = \frac{1}{((f_{m} + 1) \times \overline{f_{m}})}
\end{align*}
\end{Def1}

\subsubsection{Ranking}
After the weights have been distributed, the ranking algorithm retrieves all the metric values from the quality graph.
The values of chosen metrics $f_{m_{i}}$ (where i is a range from 0 to the number of possible metrics) are added together and multiplied by the weight $\theta$.
Similarly, those which are not chosen ($\overline{f_{m_{i}}}$) are multiplied by the weight $\rho$.
...
\label{def:ranking}
\begin{align*}
\tau = (\sum_{f_{m}}^m f_{m_{i}} \times \theta) + (\sum_{\overline{f_{m}}}^m \overline{f_{m_{i}}} \times \rho)
\end{align*}
\end{Def1}
