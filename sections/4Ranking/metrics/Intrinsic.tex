...
%~(required for: \textbf{ALL}, \textbf{DP}, \textbf{EBI})
...
\paragraph{Malformed Datatype Literals Metric}~(required for: \textbf{ALL}, \textbf{DP}, \textbf{EBI})~\\ %metric name
...
\paragraph{Literals Incompatible with Datatype Range Metric}~(required for: \textbf{ALL}, \textbf{DP}, \textbf{EBI})~\\
...
Regarding to Hogan~\todo{CL: please add to bib}\cite{hogan2010} to the most used undefined classes belong \texttt{foaf:UserGroup}, \texttt{rss:item}, \texttt{linkedct:link}, \texttt{politico:Term} .
...
Similar to the Undefined Classes metric \ref{par:undefClass} the Undefined Properties metric identifies terms in the predicate position that are used without any formal definition.
 Hogan~\todo{CL: please add to bib}\cite{hogan2010} identified the following properties that are often used without being defined:  \textit{foaf:image}, \textit{cycann:label} , \textit{foaf:tagLine}.
